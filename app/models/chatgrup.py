...
class ChatGrup(db.Model):
    ...
    room_id = db.Column(db.Integer, db.ForeignKey('chat_room_grup.id'), nullable=True)  # Sesuaikan nama tabel
    message = db.Column(db.Text, nullable=False)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)

    # Relasi dengan User dan ChatRoom
    user = db.relationship('User', backref='chat_grup_messages')
    room = db.relationship('ChatRoomGrup', backref='chat_grup_messages')
